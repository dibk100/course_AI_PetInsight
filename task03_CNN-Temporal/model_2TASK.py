...
"""
CNN backbone → 시퀀스별 특징 벡터 추출 -> 각 프레임의 의미있는 벡처 추출

Positional Encoding → 시퀀스 내 위치 정보 부여(입력값이 피처 차원, 출력층 전의 차원)

Transformer Encoder → 시퀀스 의존성 모델링

평균 풀링 + 분류기 헤드 → 행동, 상황 분류
"""


class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=200):           # 프레임 최대수가 185
        super().__init__()
...
class MultiLabelVideoTransformerClassifier(nn.Module):
    def __init__(self, num_actions, num_situations,
                 backbone_name='resnet18', pretrained=True,
                 trans_layers=4, trans_heads=8, trans_dim=512):
        super().__init__()
        
        # 1. CNN 백본
        self.shared_encoder = timm.create_model(backbone_name, pretrained=pretrained, num_classes=0)        # 출력층 제거 → 특징(feature) 추출
