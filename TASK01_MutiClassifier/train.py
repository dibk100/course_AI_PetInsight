...
def train_model(config):
    ...
    train_loader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True, num_workers=4,pin_memory=True)      # pin_memory=True :: GPU 사용시 병목 줄임
    ...
    for epoch in range(config['epochs']):
        ...
        for images, y_action, y_emotion, y_situation in train_loader:
            ...
            y_action = y_action.to(config['device'])
            y_emotion = y_emotion.to(config['device'])
            y_situation = y_situation.to(config['device'])
